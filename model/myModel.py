...
class ShortMatch(pl.LightningModule):
    ...
    def __init__(self, lr=1e-5,
        ...
        # self.model = BertForNextSentencePrediction.from_pretrained(self.hparams.pretrained, config=self.config)
        # self.model =BertForPreTraining.from_pretrained(self.hparams.pretrained)
        # self.loss_fn =
        # self.loss_fn = focal_loss( alpha=0.8, gamma=2, num_classes = 2)
        # self.loss_fn = nn.BCEWithLogitsLoss()
        # self.loss_fn=nn.MSELoss()
        # self.c = nn.Linear(self.config.hidden_size, 1)
        # elf.dropout=nn.Dropout(self.hparams.dropout)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # inputsA['input_ids'], inputsA['attention_mask'],
        ...
        #         auroc=self.auroc(F.softmax(out.view(-1,2),dim=-1), y.view(-1))
